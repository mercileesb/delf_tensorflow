...
class InferenceHelper():
	def __init__(self, config_path):
		...
		with tf.Graph().as_default():
			...
			# self.image_tf = tf.image.resize_images(self.image_tf_, [1600,1200])
...
def get_feature_from_path(image_paths, config_path):
	...
	with tf.Graph().as_default():
		...
		image_tf = tf.image.resize_images(image_tf, [800, 600])
...
def batch_get_feature(image_paths, config_path, output_dir, return_numpy_values=False):
	...
	with tf.Graph().as_default():
		...
		_, value = reader.read(filename_queue)
		image_tf = tf.image.decode_jpeg(value, channels=3)
		image_tf = tf.image.resize_images(image_tf, [1600, 1200])
		conf = tf.ConfigProto()
